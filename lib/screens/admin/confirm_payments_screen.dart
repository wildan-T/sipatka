...
class ConfirmPaymentsScreen extends StatelessWidget {
      ...
      body: StreamBuilder<List<Payment>>(
        ...
        builder: (context, snapshot) {
          ...
          if (!snapshot.hasData || snapshot.data!.isEmpty) {
            return const Center(child: Text('Tidak ada pembayaran untuk dikonfirmasi.'));
          }
  ...
  Widget _buildPaymentCard(BuildContext context, Payment payment) {
    ...
    return Card(
      ...
      child: ListTile(
        leading: CircleAvatar(child: Text(studentName.isNotEmpty ? studentName[0] : 'S')),
...
class _ConfirmationDialogState extends State<ConfirmationDialog> {
  ...
  void initState() {
    super.initState();
    _amountController.text = (widget.payment.amount + widget.payment.denda).toStringAsFixed(0);
  }
  ...
  Future<void> _onReject() async {
     if (_rejectionReasonController.text.trim().isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Alasan penolakan wajib diisi."), backgroundColor: Colors.red));
        return;
     }
     setState(() => _isProcessing = true);
     try {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
        ...
        : Form(
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                Text("Siswa: ${widget.payment.studentProfile?.studentName ?? '...'}"),
                const SizedBox(height: 16),
                const Text('Bukti Pembayaran:', style: TextStyle(fontWeight: FontWeight.bold)),
                const SizedBox(height: 8),
                if (widget.payment.proofOfPaymentUrl != null && widget.payment.proofOfPaymentUrl!.isNotEmpty)
                  ClipRRect(
                    ...
                    child: Image.network(
                      ...
                      errorBuilder: (ctx, err, st) => const Center(child: Text('Gagal muat gambar.', style: TextStyle(color: Colors.red))),
                    ),
                  )
                else
                ...
                TextFormField(
                  ...
                  decoration: const InputDecoration(labelText: 'Jumlah Diterima (Rp)', border: OutlineInputBorder()),
                  validator: (v) => (v == null || v.isEmpty || double.tryParse(v) == null) ? 'Jumlah tidak valid' : null,
                ),
                const SizedBox(height: 16),
                TextFormField(
                  controller: _rejectionReasonController,
                  decoration: const InputDecoration(labelText: 'Alasan Penolakan (jika ditolak)', border: OutlineInputBorder()),
                ),
              ],
            ),
          ),
      ...
      actions: [
        ...
        TextButton(onPressed: _isProcessing ? null : _onReject, child: const Text('Tolak', style: TextStyle(color: Colors.red))),
        ElevatedButton(onPressed: _isProcessing ? null : _onConfirm, child: const Text('Konfirmasi')),
      ],
    );
  }
}